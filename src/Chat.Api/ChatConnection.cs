...
using static Chat.Api.Internals.MessageSerialization;
...
public sealed class ChatConnection
{
    ...
    {
        keepaliveTimeSpan = keepaliveTimeSpan == TimeSpan.Zero ? TimeSpan.FromSeconds(5) : keepaliveTimeSpan;
    ...
    private (IReadOnlyList<IMessage> Messages, SequencePosition Consumed) ParseMessages(ReadOnlySequence<byte> buffer)
    {
